...
export class MeasureComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.measureService
      ...
      .subscribe(
        (res: HttpResponse<IMeasure[]>) => this.paginateMeasures(res.body, res.headers),
  ...
  registerChangeInMeasures() {
    this.eventSubscriber = this.eventManager.subscribe('measureListModification', response => this.loadAll());
  }
  ...
  protected paginateMeasures(data: IMeasure[], headers: HttpHeaders) {
